...
возраст и стадия на которой находится растение.''')
        ...
        # general_text.image_create(tk.END, image=images[1])
        general_text.insert(END,
                            f'''\nСтадии роста:
...
После того, как подключили схему, то нужно 1 раз самостоятельно полить грядки из канистры, а дальше авто полив справится сам (если почва начнет сохнуть, то поднимите немного время работы таймера).
...
А вот бафы на руды или дерево, самые классные, но прокачивать их к чистым, не лучшая идея, так как 4 чая на 60% не намного хуже чем 1 чай на 100%.
